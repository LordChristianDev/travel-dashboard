...
const Dashboard = ({ loaderData }: Route.ComponentProps) => {
	...
	const renderUserAndTrips = usersAndTrips.map((item, index) => {
		const { title, dataSource, field, headerText } = item;

		const renderTemplate = (props: UserData) => {
			const { imageUrl, name } = props;

			return (
				<div className="flex items-center gap-1.5 px-4">
					<img src={imageUrl} alt="user" className="rounded-full size-8 aspect-square" referrerPolicy="no-referrer" />
					<span>{name}</span>
				</div>
			);
		}
		...
		return (
			<div key={index + title} className="flex flex-col gap-5">
